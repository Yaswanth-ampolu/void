...
export class PinnacleAIMainUpdateService extends Disposable implements IPinnacleAIUpdateService {
	...
	async check(explicit: boolean): Promise<PinnacleAICheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart PinnacleAI to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<PinnacleAICheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/pinnacleaieditor/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of PinnacleAI is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'PinnacleAI is up-to-date!'
					}
				}
				else {
