...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/pinnacleaiSettingsTypes.js';
...
import { PinnacleAIFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export const IChatThreadService = createDecorator<IChatThreadService>('pinnacleaiChatThreadService');
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	// should probably re-use code from pinnacleai/src/vs/base/common/marshalling.ts instead. but this is simple enough
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Pinnacleai)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Pinnacleai), cancel that last tool
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const pinnacleaiFileSnapshot = checkpointMessage.pinnacleaiFileSnapshotOfURI ? checkpointMessage.pinnacleaiFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { pinnacleaiFileSnapshot, } }

		const userModifiedPinnacleaiFileSnapshot = fsPath in checkpointMessage.userModifications.pinnacleaiFileSnapshotOfURI ? checkpointMessage.userModifications.pinnacleaiFileSnapshotOfURI[fsPath] ?? null : null
		return { pinnacleaiFileSnapshot: userModifiedPinnacleaiFileSnapshot ?? pinnacleaiFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const pinnacleaiFileSnapshotOfURI: { [fsPath: string]: PinnacleAIFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._pinnacleaiModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { pinnacleaiFileSnapshot: oldPinnacleaiFileSnapshot } = res
			...
			const pinnacleaiFileSnapshot = this._editCodeService.getPinnacleaiFileSnapshot(URI.file(fsPath))
			if (oldPinnacleaiFileSnapshot === pinnacleaiFileSnapshot) continue
			pinnacleaiFileSnapshotOfURI[fsPath] = pinnacleaiFileSnapshot
		}
		...
		return { pinnacleaiFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { pinnacleaiFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			pinnacleaiFileSnapshotOfURI: pinnacleaiFileSnapshotOfURI ?? {},
			userModifications: { pinnacleaiFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getPinnacleaiFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			pinnacleaiFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { pinnacleaiFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.pinnacleaiFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { pinnacleaiFileSnapshotOfURI: pinnacleaiFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { pinnacleaiFileSnapshot } = res
					if (!pinnacleaiFileSnapshot) continue
					this._editCodeService.restorePinnacleaiFileSnapshot(URI.file(fsPath), pinnacleaiFileSnapshot)
					break
				}
			}
		}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._pinnacleaiModelService.getModelSafe(uri)
