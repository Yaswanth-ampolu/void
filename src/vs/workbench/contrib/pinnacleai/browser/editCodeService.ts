...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._pinnacleaiModelService.initializeModel(model.uri)
	...
	// 		message: `Pinnacleai Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'pinnacleai.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Pinnacleai's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(PINNACLEAI_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/pinnacleaieditor/pinnacleai/issues/new) it.` : undefined
	// 	})
	// }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'pinnacleai-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'pinnacleai-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'pinnacleai-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'pinnacleai-greenBG', {
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'pinnacleai-redBG'
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('Pinnacleai 1') }
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		...
		const elt: IUndoRedoElement = {
			...
			resource: uri,
			label: 'Pinnacleai Agent',
			code: 'undoredo.editCode',
			undo: async () => { opts?.onWillUndo?.(); await this._restorePinnacleaiFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restorePinnacleaiFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentPinnacleaiFileSnapshot(uri)
			await this._pinnacleaiModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getPinnacleaiFileSnapshot(uri: URI) {
		return this._getCurrentPinnacleaiFileSnapshot(uri)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Pinnacleai: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._pinnacleaiModelService.initializeModel(uri)
		await this._pinnacleaiModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Pinnacleai: diff.type not recognized on: ${from}`)
		}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Pinnacleai: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	private _undoHistory(uri: URI) {
		const { model } = this._pinnacleaiModelService.getModel(uri)
		if (!model) return
		const history = this._undoRedoService.getElements(uri)
		const lastElt = history.past[history.past.length - 1]
		if (lastElt?.code === 'undoredo.editCode') {
	...
	private _stopIfStreaming(diffZone: DiffZone) {
		if (diffZone._streamState.isStreaming) {
			const { streamRequestIdRef } = diffZone._streamState
			if (streamRequestIdRef.current) {
				this._llmMessageService.abort(streamRequestIdRef.current)
			}
		}
	...
	public acceptDiff({ diffid }: { diffid: number }) {
		...
		// if deletion, delete the original code
		if (diff.type === 'deletion') {
			this._writeURIText(uri, '',
				{ startLineNumber: diff.startLine, startColumn: 1, endLineNumber: diff.startLine, endColumn: Number.MAX_SAFE_INTEGER },
				{ shouldRealignDiffAreas: true }
			)
		}
		// if insertion, do nothing
		else if (diff.type === 'insertion') {
			// do nothing
		}
		// if edit, delete the original code
	...
	public rejectDiff({ diffid }: { diffid: number }) {
		...
		// if deletion, do nothing
		if (diff.type === 'deletion') {
			// do nothing
		}
		// if insertion, delete the inserted code
		else if (diff.type === 'insertion') {
			this._writeURIText(uri, '',
				{ startLineNumber: diff.startLine, startColumn: 1, endLineNumber: diff.endLine, endColumn: Number.MAX_SAFE_INTEGER },
				{ shouldRealignDiffAreas: true }
			)
		}
		// if edit, delete the inserted code
		else if (diff.type === 'edit') {
			this._writeURIText(uri, '',
				{ startLineNumber: diff.startLine, startColumn: 1, endLineNumber: diff.endLine, endColumn: Number.MAX_SAFE_INTEGER },
				{ shouldRealignDiffAreas: true }
	...
	public acceptOrRejectAllDiffAreas({ uri, removeCtrlKs, behavior, _addToHistory }: { uri: URI, removeCtrlKs: boolean, behavior: 'accept' | 'reject', _addToHistory?: boolean }) {
		if (_addToHistory === undefined) _addToHistory = true

		const { onFinishEdit } = _addToHistory ? this._addToHistory(uri) : { onFinishEdit: () => { } }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			for (const diffid in diffArea._diffOfId) {
				if (behavior === 'accept') this.acceptDiff({ diffid: parseInt(diffid) })
				else if (behavior === 'reject') this.rejectDiff({ diffid: parseInt(diffid) })
			}
		}

		// delete all diffzones
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			const diffArea = this.diffAreaOfId[diffareaid]
			if (diffArea.type === 'DiffZone') {
				this._deleteDiffZone(diffArea)
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		this._domNode.className = 'void-accept-reject-widget';
		this._domNode.style.display = 'flex';
		this._domNode.style.alignItems = 'center';
		this._domNode.style.position = 'absolute';
		this._domNode.style.zIndex = '50';
		...
		const createButton = (text: string, keybindingId: string, onClick: () => void, isAccept: boolean) => {
			...
			button.style.border = `1px solid ${isAccept ? acceptBorder : rejectBorder}`;
			button.style.backgroundColor = isAccept ? acceptBg : rejectBg;
			button.style.cursor = 'pointer';

			const keybinding = this._keybindingService.lookupKeybinding(keybindingId);
			const keybindingLabel = keybinding ? keybinding.getLabel() : '';

			button.textContent = `${text} (${keybindingLabel})`;
			this.onclick(button, (e) => {
				e.preventDefault();
				e.stopPropagation();
				onClick();
			});
		...
		this._acceptButton = createButton('Accept', PINNACLEAI_ACCEPT_DIFF_ACTION_ID, this._opts.onAccept, true);
		this._rejectButton = createButton('Reject', PINNACLEAI_REJECT_DIFF_ACTION_ID, this._opts.onReject, false);

		this._domNode.appendChild(this._acceptButton);
		this._domNode.appendChild(this._rejectButton);

		this._opts.editor.addOverlayWidget(this);
	}

	override dispose(): void {
		this._opts.editor.removeOverlayWidget(this);
		super.dispose();
	}

	getId(): string {
		return `void.acceptRejectWidget.${this._opts.diffid}`;
	}
	...
	getPosition(): { preference: null } {
		const { editor, startLine, offsetLines } = this._opts;
		const editorLayout = editor.getLayoutInfo();
		const topForLine = editor.getTopForLineNumber(startLine, true);
		const editorScrollTop = editor.getScrollTop();

		const finalTop = topForLine - editorScrollTop + (offsetLines * editor.getOption(EditorOption.lineHeight));

		this._domNode.style.top = `${finalTop}px`;
		this._domNode.style.left = `${editorLayout.contentLeft}px`;
