...
import { PINNACLEAI_VIEW_CONTAINER_ID, PINNACLEAI_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { PINNACLEAI_TOGGLE_SETTINGS_ACTION_ID } from './pinnacleaiSettingsPane.js';
...
const PINNACLEAI_OPEN_SIDEBAR_ACTION_ID = 'pinnacleai.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: PINNACLEAI_OPEN_SIDEBAR_ACTION_ID, title: localize2('pinnacleaiOpenSidebar', 'Pinnacleai: Open Sidebar'), f1: true });
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PINNACLEAI_CTRL_L_ACTION_ID,
			f1: true,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(PINNACLEAI_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(PINNACLEAI_OPEN_SIDEBAR_ACTION_ID)
		}
...
const PINNACLEAI_CMD_SHIFT_L_ACTION_ID = 'pinnacleai.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PINNACLEAI_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(PINNACLEAI_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'pinnacleai.settingsAction',
			title: `Pinnacleai's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(PINNACLEAI_TOGGLE_SETTINGS_ACTION_ID)
	}
})
