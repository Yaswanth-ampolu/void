...
function saveStylesFile() {
	setTimeout(() => {
		try {
			const pathToCssFile = findDesiredPathFromLocalPath('./src/vs/workbench/contrib/pinnacleai/browser/react/src2/styles.css', __dirname);
...
if (isWatch) {
	...
	if (!fs.existsSync('src2')) {
		try {
			...
			execSync(
				'npx scope-tailwind ./src -o src2/ -s pinnacleai-scope -c styles.css -p "pinnacleai-"',
				{ stdio: 'inherit' }
			);
	...
	execSync('npx scope-tailwind ./src -o src2/ -s pinnacleai-scope -c styles.css -p "pinnacleai-"', { stdio: 'inherit' });
