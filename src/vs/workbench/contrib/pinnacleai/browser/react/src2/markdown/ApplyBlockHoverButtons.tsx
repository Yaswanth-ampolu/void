...
import { isFeatureNameDisabled } from '../../../../common/pinnacleaiSettingsTypes.js';
import { URI } from '../../../../../../../base/common/uri.js';
import { FileSymlink, LucideIcon, RotateCw, Terminal } from 'lucide-react';
import { Check, X, Square, Copy, Play } from 'lucide-react';
import { getBasename, ListableToolItem, pinnacleaiOpenFileFn, ToolChildrenWrapper } from '../sidebar-tsx/SidebarChat.js';
...
export const IconShell1 = ({ onClick, Icon, disabled, className, ...props }: IconButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  ...
  return <button
    ...
    // border border-pinnacleai-border-1 rounded
    className={` pinnacleai-size-[18px] pinnacleai-p-[2px] pinnacleai-flex pinnacleai-items-center pinnacleai-justify-center pinnacleai-text-sm pinnacleai-text-pinnacleai-fg-3 hover:pinnacleai-brightness-110 disabled:pinnacleai-opacity-50 disabled:pinnacleai-cursor-not-allowed ${
...
export const JumpToFileButton = ({ uri, ...props }: {uri: URI | 'current';} & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  ...
  <IconShell1
    Icon={FileSymlink}
    onClick={() => {
      pinnacleaiOpenFileFn(uri, accessor);
    }}
...
export const useApplyStreamState = ({ applyBoxId }: {applyBoxId: string;}) => {
  const accessor = useAccessor();
  const pinnacleaiCommandBarService = accessor.get('IPinnacleAICommandBarService');

  const getStreamState = useCallback(() => {
    const uri = getUriBeingApplied(applyBoxId);
    if (!uri) return 'idle-no-changes';
    return pinnacleaiCommandBarService.getStreamState(uri);
  }, [pinnacleaiCommandBarService, applyBoxId]);
...
export const StatusIndicator = ({ indicatorColor, title, className, ...props }: {indicatorColor: IndicatorColor;title?: React.ReactNode;className?: string;} & React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <div className={`pinnacleai-flex pinnacleai-flex-row pinnacleai-text-pinnacleai-fg-3 pinnacleai-text-xs pinnacleai-items-center pinnacleai-gap-1.5 ${className}`} {...props}>
			{title && <span className="pinnacleai-opacity-80">{title}</span>}
			<div
        className={` pinnacleai-size-1.5 pinnacleai-rounded-full pinnacleai-border ${
        indicatorColor === 'dark' ? "pinnacleai-bg-[rgba(0,0,0,0)] pinnacleai-border-pinnacleai-border-1" :
        indicatorColor === 'orange' ? "pinnacleai-bg-orange-500 pinnacleai-border-orange-500 pinnacleai-shadow-[0_0_4px_0px_rgba(234,88,12,0.6)]" :
        indicatorColor === 'green' ? "pinnacleai-bg-green-500 pinnacleai-border-green-500 pinnacleai-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" :
        indicatorColor === 'yellow' ? "pinnacleai-bg-yellow-500 pinnacleai-border-yellow-500 pinnacleai-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" : "pinnacleai-bg-pinnacleai-border-1 pinnacleai-border-pinnacleai-border-1"} `} />
...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: {tooltipName: string;color?: IndicatorColor;position?: PlacesType;offset?: number;}) => ({
  'data-tooltip-id': color === 'orange' ? `pinnacleai-tooltip-orange` : color === 'green' ? 'pinnacleai-tooltip-green' : 'pinnacleai-tooltip',
...
export const useEditToolStreamState = ({ applyBoxId, uri }: {applyBoxId: string;uri: URI;}) => {
  const accessor = useAccessor();
  const pinnacleaiCommandBarService = accessor.get('IPinnacleAICommandBarService');
  const [streamState, setStreamState] = useState(pinnacleaiCommandBarService.getStreamState(uri));
  // listen for stream updates on this box
  useCommandBarURIListener(useCallback((uri_) => {
    const shouldUpdate = uri.fsPath === uri_.fsPath;
    if (shouldUpdate) {setStreamState(pinnacleaiCommandBarService.getStreamState(uri));}
  }, [pinnacleaiCommandBarService, applyBoxId, uri]));
...
const ApplyButtonsForEdit = ({
  ...
  const onClickSubmit = useCallback(async () => {
    ...
    if (!applyDonePromise) {
      notificationService.info(`Pinnacle Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`);
    }
    ...
    applyDonePromise?.catch((e) => {
      ...
      notificationService.info(`Pinnacle Error: There was a problem running Apply: ${e}.`);
...
export const BlockCodeApplyWrapper = ({
  ...
  <ListableToolItem
    name={<span className="pinnacleai-not-italic">{getBasename(uri.fsPath)}</span>}
    isSmall={true}
    showDot={false}
  ...
  return <div className="pinnacleai-border pinnacleai-border-pinnacleai-border-3 pinnacleai-rounded pinnacleai-overflow-hidden pinnacleai-bg-pinnacleai-bg-3 pinnacleai-my-1">
		{/* header */}
		<div className=" pinnacleai-select-none pinnacleai-flex pinnacleai-justify-between pinnacleai-items-center pinnacleai-py-1 pinnacleai-px-2 pinnacleai-border-b pinnacleai-border-pinnacleai-border-3 pinnacleai-cursor-default">
			<div className="pinnacleai-flex pinnacleai-items-center">
				<StatusIndicatorForApplyButton uri={uri} applyBoxId={applyBoxId} />
				<span className="pinnacleai-text-[13px] pinnacleai-font-light pinnacleai-text-pinnacleai-fg-3">
					{name}
				</span>
			</div>
			<div className={`${canApply ? "" : "pinnacleai-hidden"} pinnacleai-flex pinnacleai-items-center pinnacleai-gap-1`}>
