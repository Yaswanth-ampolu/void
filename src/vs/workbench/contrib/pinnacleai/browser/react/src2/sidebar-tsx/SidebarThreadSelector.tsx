...
export const PastThreadsList = ({ className = '' }: {className?: string;}) => {
  ...
  if (!allThreads) {
    return <div key="error" className="pinnacleai-p-1">{`Error accessing chat history.`}</div>;
  }
  ...
  return (
    <div className={`pinnacleai-flex pinnacleai-flex-col pinnacleai-mb-2 pinnacleai-gap-2 pinnacleai-w-full pinnacleai-text-nowrap pinnacleai-text-pinnacleai-fg-3 pinnacleai-select-none pinnacleai-relative ${className}`}>
			{displayThreads.length === 0 // this should never happen
      ? <></> :
      displayThreads.map((threadId, i) => {
        const pastThread = allThreads[threadId];
        if (!pastThread) {
          return <div key={i} className="pinnacleai-p-1">{`Error accessing chat history.`}</div>;
        }
      ...
      <div
        className="pinnacleai-text-pinnacleai-fg-3 pinnacleai-opacity-80 hover:pinnacleai-opacity-100 hover:pinnacleai-brightness-115 pinnacleai-cursor-pointer pinnacleai-p-1 pinnacleai-text-xs"
...
const DuplicateButton = ({ threadId }: {threadId: string;}) => {
  ...
  return <IconShell1
    Icon={Copy}
    className="pinnacleai-size-[11px]"
...
const TrashButton = ({ threadId }: {threadId: string;}) => {
  ...
  <div className="pinnacleai-flex pinnacleai-flex-nowrap pinnacleai-text-nowrap pinnacleai-gap-1">
			<IconShell1
      Icon={X}
      className="pinnacleai-size-[11px]"
...

{
  ...
  // 	}}
  // 	toolTipName={`Copy As Pinnacle Chat`}
  // />
  ...
  // data-tooltip-place='top'
  >
		<span className="pinnacleai-opacity-60">{numMessages}</span>
		{` `}
  ...
  return <div
    ...
    onMouseLeave={() => setHoveredIdx(null)}>
		...
		<div className="pinnacleai-flex pinnacleai-items-center pinnacleai-justify-between pinnacleai-gap-1">
			<span className="pinnacleai-flex pinnacleai-items-center pinnacleai-gap-2 pinnacleai-min-w-0 pinnacleai-overflow-hidden">
				{/* spinner */}
				{isRunning === 'LLM' || isRunning === 'tool' || isRunning === 'idle' ? <LoaderCircle className="pinnacleai-animate-spin pinnacleai-bg-pinnacleai-stroke-1 pinnacleai-flex-shrink-0 pinnacleai-flex-grow-0" size={14} /> :

        isRunning === 'awaiting_user' ? <MessageCircleQuestion className="pinnacleai-bg-pinnacleai-stroke-1 pinnacleai-flex-shrink-0 pinnacleai-flex-grow-0" size={14} /> :

        null}
				{/* name */}
				<span className="pinnacleai-truncate pinnacleai-overflow-hidden pinnacleai-text-ellipsis"
        data-tooltip-id='pinnacleai-tooltip'
        ...
        data-tooltip-place='top'>
          {firstMsg}</span>
			...
			<div className="pinnacleai-flex pinnacleai-items-center pinnacleai-gap-x-1 pinnacleai-opacity-60">
				{idx === hoveredIdx ?
        <>
						{/* trash icon */}
