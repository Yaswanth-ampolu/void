...
import { getBasename, getRelative, pinnacleaiOpenFileFn } from '../sidebar-tsx/SidebarChat.js';
...
const LatexRender = ({ latex }: {latex: string;}) => {
  return <span className="pinnacleai-katex-error pinnacleai-text-red-500">{latex}</span>;
  // try {
  // 	let formula = latex;
...
const Codespan = ({ text, className, onClick, tooltip }: {text: string;className?: string;onClick?: () => void;tooltip?: string;}) => {
  ...
  return <code
    className={`pinnacleai-font-mono pinnacleai-font-medium pinnacleai-rounded-sm pinnacleai-bg-pinnacleai-bg-1 pinnacleai-px-1 ${className}`}
    onClick={onClick}
    {...tooltip ? {
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: {text: string;rawText: string;chatMessageLocation: ChatMessageLocation;}) => {
  ...
  const onClick = () => {
    if (!link) return;
    // Use the updated pinnacleaiOpenFileFn to open the file and handle selection
    if (link.selection)
    pinnacleaiOpenFileFn(link.uri, accessor, [link.selection.startLineNumber, link.selection.endLineNumber]);else

    pinnacleaiOpenFileFn(link.uri, accessor);
  };

  return <Codespan
    text={displayText}
    onClick={onClick}
    className={link ? "pinnacleai-underline hover:pinnacleai-brightness-90 pinnacleai-transition-all pinnacleai-duration-200 pinnacleai-cursor-pointer" : ""}
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: {token: Token | string;inPTag?: boolean;codeURI?: URI;chatMessageLocation?: ChatMessageLocation;tokenIdx: string;} & RenderTokenOptions): React.ReactNode => {
  ...
  if (t.type === 'table') {
    ...
    // return (
    // 	<div>
    // 		<table className={'min-w-full border border-pinnacleai-bg-2'}>
    // 			<thead>
    // 				<tr className='bg-pinnacleai-bg-1'>
    // 					{t.header.map((cell: any, index: number) => (
    // 						<th
    // 							key={index}
    // 							className='px-4 py-2 border border-pinnacleai-bg-2 font-semibold'
    ...
    // 					<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-pinnacleai-bg-1'}>
    // 						{row.map((cell: any, cellIndex: number) => (
    // 							<td
    // 								key={cellIndex}
    // 								className={'px-4 py-2 border border-pinnacleai-bg-2'}
  ...
  if (t.type === 'paragraph') {
    ...
    if (latexSegments.length !== 0) {
      if (inPTag) {
        return <span className="pinnacleai-block">{latexSegments}</span>;
      }
    ...
    if (inPTag) return <span className="pinnacleai-block">{contents}</span>;
    return <p>{contents}</p>;
  }
  ...
  if (t.type === 'link') {
    return (
      <a
        ...
        className="pinnacleai-underline pinnacleai-cursor-pointer hover:pinnacleai-brightness-90 pinnacleai-transition-all pinnacleai-duration-200 pinnacleai-text-pinnacleai-fg-2">
  ...
  // default
  return (
    <div className="pinnacleai-bg-orange-50 pinnacleai-rounded-sm pinnacleai-overflow-hidden pinnacleai-p-2">
			<span className="pinnacleai-text-sm pinnacleai-text-orange-500">Unknown token rendered...</span>
		</div>);
