...
export const PinnacleInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	return <>
		<textarea
			...
			className={`w-full resize-none max-h-[500px] overflow-y-auto text-pinnacleai-fg-1 placeholder:text-pinnacleai-fg-3 ${className}`}
			style={{
		...
		{isMenuOpen && (
			<div
				...
				className="z-[100] border-pinnacleai-border-3 bg-pinnacleai-bg-2-alt border rounded shadow-lg flex flex-col overflow-hidden"
				style={{
					position: strategy,
					top: y ?? 0,
					left: x ?? 0,
			...
			>
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="px-2 py-1 text-pinnacleai-fg-1 bg-pinnacleai-bg-2-alt border-b border-pinnacleai-border-3 sticky top-0 bg-pinnacleai-bg-1 z-10 select-none pointer-events-none">
					{optionText ?
				...
				<div className='max-h-[400px] w-full max-w-full overflow-y-auto overflow-x-auto'>
					<div className="w-max min-w-full flex flex-col gap-0 text-nowrap flex-nowrap">
						{options.length === 0 ?
							<div className="text-pinnacleai-fg-3 px-3 py-0.5">No results found</div>
							: options.map((o, oIdx) => {
								...
								return (
									...
									<div
										...
										className={`
											...
											${oIdx === optionIdx ? 'bg-blue-500 text-white/80' : 'bg-pinnacleai-bg-2-alt text-pinnacleai-fg-1'}
										`}
...
export const PinnacleSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	...
	return (
		<input
			...
			placeholder={placeholder}
			disabled={disabled}
			className={`w-full resize-none bg-pinnacleai-bg-1 text-pinnacleai-fg-1 placeholder:text-pinnacleai-fg-3 border border-pinnacleai-border-2 focus:border-pinnacleai-border-1
...
export const PinnacleInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	...
	return <WidgetComponent
		className='
			bg-pinnacleai-bg-1
			@@pinnacleai-force-child-placeholder-pinnacleai-fg-1
		'
		ctor={InputBox}
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-pinnacleai-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
						...
						<div
							className={`absolute left-0 ${size === 'xxs' ? 'h-0.5' :
								size === 'xs' ? 'h-1' :
									size === 'sm' ? 'h-1.5' :
										size === 'sm+' ? 'h-2' : 'h-2.5'
								} bg-pinnacleai-fg-1 rounded-full`}
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-pinnacleai-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-pinnacleai-fg-1`}
	...
	return (
		<div className={`inline-block relative ${className}`}>
			...
			{isOpen && (
				<div
					ref={refs.setFloating}
					className="z-[100] bg-pinnacleai-bg-1 border-pinnacleai-border-3 border rounded shadow-lg"
					style={{
						position: strategy,
						top: y ?? 0,
						left: x ?? 0,
...
export const _PinnacleSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className }: {
	...
	return <WidgetComponent
		className={`
			...
			@@[&_select]:!pinnacleai-text-pinnacleai-fg-3
			@@[&_select]:!pinnacleai-text-xs
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-pinnacleai-bg-3'>
		<WidgetComponent
...
const SingleDiffEditor = ({ block, lang }: { block: ExtractedSearchReplaceBlock, lang: string | undefined }) => {
	...
	return (
		<div className="w-full bg-pinnacleai-bg-3 @@bg-editor-style-override" ref={divRef} />
	);
};
...
export const PinnacleDiffEditor = ({ uri, searchReplaceBlocks, language }: { uri?: any, searchReplaceBlocks: string, language?: string }) => {
	...
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-pinnacleai-fg-4 text-sm">No changes found</div>;
	}
		...
		<div className="w-full flex flex-col gap-2">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && (
						<div className="text-pinnacleai-fg-4 text-xs mb-1 px-1">
