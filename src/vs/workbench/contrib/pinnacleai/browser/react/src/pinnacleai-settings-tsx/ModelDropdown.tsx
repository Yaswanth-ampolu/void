...
import { FeatureName, featureNames, isFeatureNameDisabled, ModelSelection, modelSelectionsEqual, ProviderName, providerNames, SettingsOfProvider } from '../../../../../../../workbench/contrib/pinnacleai/common/pinnacleaiSettingsTypes.js'
import { useSettingsState, useRefreshModelState, useAccessor } from '../util/services.js'
import { _PinnacleSelectBox, PinnacleCustomDropdownBox } from '../util/inputs.js'
import { SelectBox } from '../../../../../../../base/browser/ui/selectBox/selectBox.js'
import { IconWarning } from '../sidebar-tsx/SidebarChat.js'
import { PINNACLEAI_OPEN_SETTINGS_ACTION_ID, PINNACLEAI_TOGGLE_SETTINGS_ACTION_ID } from '../../../../../../contrib/pinnacleai/browser/pinnacleaiSettingsPane.js'
import { modelFilterOfFeatureName, ModelOption } from '../../../../../../../workbench/contrib/pinnacleai/common/pinnacleaiSettingsService.js'
...
const ModelSelectBox = ({ options, featureName, className }: { options: ModelOption[], featureName: FeatureName, className: string }) => {
	...
	const selection = pinnacleaiSettingsService.state.modelSelectionOfFeature[featureName]
	const selectedOption = selection ? pinnacleaiSettingsService.state._modelOptions.find(v => modelSelectionsEqual(v.selection, selection))! : options[0]

	const onChangeOption = useCallback((newOption: ModelOption) => {
		pinnacleaiSettingsService.setModelSelectionOfFeature(featureName, newOption.selection)
	}, [pinnacleaiSettingsService, featureName])
...
export const ModelDropdown = ({ featureName, className }: { featureName: FeatureName, className: string }) => {
	...
	const openSettings = () => { commandService.executeCommand(PINNACLEAI_OPEN_SETTINGS_ACTION_ID); };
