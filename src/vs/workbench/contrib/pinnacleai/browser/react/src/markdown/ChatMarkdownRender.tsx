...
const Codespan = ({ text, className, onClick, tooltip }: { text: string, className?: string, onClick?: () => void, tooltip?: string }) => {
	...
	return <code
		className={`font-mono font-medium rounded-sm bg-pinnacleai-bg-1 px-1 ${className}`}
		onClick={onClick}
		{...tooltip ? {
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: { text: string, rawText: string, chatMessageLocation: ChatMessageLocation }) => {
	...
	const onClick = () => {
		...
		if (link.selection)
			pinnacleaiOpenFileFn(link.uri, accessor, [link.selection.startLineNumber, link.selection.endLineNumber]);
		else
	...
	if (t.type === 'link') {
		return (
			<a
				...
				className='underline cursor-pointer hover:brightness-90 transition-all duration-200 text-pinnacleai-fg-2'
			>
				{t.text}
			</a>
		)
