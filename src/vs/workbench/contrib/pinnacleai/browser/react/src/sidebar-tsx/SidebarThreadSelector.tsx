...
export const PastThreadsList = ({ className = '' }: { className?: string }) => {
	...
	return (
		<div className={`flex flex-col mb-2 gap-2 w-full text-nowrap text-pinnacleai-fg-3 select-none relative ${className}`}>
			...
			{hasMoreThreads && !showAll && (
				<div
					className="text-pinnacleai-fg-3 opacity-80 hover:opacity-100 hover:brightness-115 cursor-pointer p-1 text-xs"
	...
	>
		<div className="flex items-center justify-between gap-1">
			<span className="flex items-center gap-2 min-w-0 overflow-hidden">
				{/* spinner */}
				{isRunning === 'LLM' || isRunning === 'tool' || isRunning === 'idle' ? <LoaderCircle className="animate-spin bg-pinnacleai-stroke-1 flex-shrink-0 flex-grow-0" size={14} />
					:
					isRunning === 'awaiting_user' ? <MessageCircleQuestion className="bg-pinnacleai-stroke-1 flex-shrink-0 flex-grow-0" size={14} />
						:
						null}
				{/* name */}
