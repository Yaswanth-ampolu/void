...
export class ToolsService implements IToolsService {
	...
	constructor(
		...
		@IPinnacleAIModelService pinnacleaiModelService: IPinnacleAIModelService,
		...
		this.callTool = {
			read_file: async ({ uri, startLine, endLine, pageNumber }) => {
				await pinnacleaiModelService.initializeModel(uri)
				const { model } = await pinnacleaiModelService.getModelSafe(uri)
		...
		this.stringOfResult = {
			...
			search_in_file: (params, result) => {
				const { model } = pinnacleaiModelService.getModel(params.uri)
			...
			edit_file: (params, result) => {
				const lintErrsString = (
					this.pinnacleaiSettingsService.state.globalSettings.includeToolLintErrors ?
			...
			run_command: (params, result) => {
				...
				if (resolveReason.type === 'timeout') {
					return `${result_}\nTerminal command ran, but was automatically killed by PinnacleAI after ${MAX_TERMINAL_INACTIVE_TIME}s of inactivity and did not finish successfully. To try with more time, open a persistent terminal and run the command there.`
				}
