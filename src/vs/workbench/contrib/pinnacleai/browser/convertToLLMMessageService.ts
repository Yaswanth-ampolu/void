...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .pinnacleairules file):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	// Read .pinnacleairules files from workspace folders
	private _getPinnacleaiRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let pinnacleaiRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.pinnacleairules')
				const { model } = this.pinnacleaiModelService.getModel(uri)
				if (!model) continue
				pinnacleaiRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return pinnacleaiRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .pinnacleairules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.pinnacleaiSettingsService.state.globalSettings.aiInstructions;
		const pinnacleaiRulesFileContent = this._getPinnacleaiRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (pinnacleaiRulesFileContent) ans.push(pinnacleaiRulesFileContent)
		return ans.join('\n\n')
	}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.pinnacleaiSettingsService.state
		...
		const modelSelectionOptions = this.pinnacleaiSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.pinnacleaiSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.pinnacleaiSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
