...
class PinnacleAISettingsInput extends EditorInput {

	static readonly ID: string = 'workbench.input.pinnacleai.settings';

	static readonly RESOURCE = URI.from({ // I think this scheme is invalid, it just shuts up TS
		scheme: 'pinnacleai',  // Custom scheme for our editor (try Schemas.https)
		path: 'settings'
	})
	readonly resource = PinnacleAISettingsInput.RESOURCE;
	...
	override get typeId(): string {
		return PinnacleAISettingsInput.ID;
	}

	override getName(): string {
		return nls.localize('pinnacleaiSettingsInputsName', 'PinnacleAI\'s Settings');
	}
...
class PinnacleAISettingsPane extends EditorPane {
	...
	constructor(
		...
		super(PinnacleAISettingsPane.ID, group, telemetryService, themeService, storageService);
	}
...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(PinnacleAISettingsPane, PinnacleAISettingsPane.ID, nls.localize('PinnacleAISettingsPane', "PinnacleAI\'s Settings Pane")),
	[new SyncDescriptor(PinnacleAISettingsInput)]
);


// register the gear on the top right
export const PINNACLEAI_TOGGLE_SETTINGS_ACTION_ID = 'workbench.action.togglePinnacleAISettings'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PINNACLEAI_TOGGLE_SETTINGS_ACTION_ID,
			title: nls.localize2('pinnacleaiSettings', "PinnacleAI: Toggle Settings"),
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// if is open, close it
		const openEditors = editorService.findEditors(PinnacleAISettingsInput.RESOURCE); // should only have 0 or 1 elements...
		...
		// else open it
		const input = instantiationService.createInstance(PinnacleAISettingsInput);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: PINNACLEAI_OPEN_SETTINGS_ACTION_ID,
			title: nls.localize2('pinnacleaiSettingsAction2', "PinnacleAI: Open Settings"),
			f1: true,
			icon: Codicon.settingsGear,
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const openEditors = editorService.findEditors(PinnacleAISettingsInput.RESOURCE);
		if (openEditors.length > 0) {
			await editorService.closeEditors(openEditors);
		}
