...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('pinnacleaiGenerateCommitMessageService');

const loadingContextKey = 'pinnacleaiSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private pinnacleaiSCM: IPinnacleAISCMService
	...
	constructor(
		...
		this.pinnacleaiSCM = ProxyChannel.toService<IPinnacleAISCMService>(mainProcessService.getChannel('pinnacleai-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.pinnacleaiSCM.gitStat(path),
					this.pinnacleaiSCM.gitSampledDiffs(path),
					this.pinnacleaiSCM.gitBranch(path),
					this.pinnacleaiSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.pinnacleaiSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.pinnacleaiSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.pinnacleaiSettingsService.state.overridesOfModel
		...
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				onFinalMessage: (params: { fullText: string }) => {
					const match = params.fullText.match(/<o>([\s\S]*?)<\/output>/i)
				...
				logging: { loggingName: 'PinnacleAI SCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('pinnacleaiFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'pinnacleai.generateCommitMessageAction',
			title: localize2('pinnacleaiCommitMessagePrompt', 'PinnacleAI: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('pinnacleaiCommitMessagePromptTooltip', 'PinnacleAI: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'pinnacleai.loadingGenerateCommitMessageAction',
			title: localize2('pinnacleaiCommitMessagePromptCancel', 'PinnacleAI: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('pinnacleaiCommitMessagePromptCancelTooltip', 'PinnacleAI: Cancel Commit Message Generation'),
