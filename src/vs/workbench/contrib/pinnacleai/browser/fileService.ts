...
class FilePromptActionService extends Action2 {
	private static readonly PINNACLEAI_COPY_FILE_PROMPT_ID = 'pinnacleai.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.PINNACLEAI_COPY_FILE_PROMPT_ID,
			title: localize2('pinnacleaiCopyPrompt', 'Pinnacleai: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await pinnacleaiModelService.getModelSafe(uri)).model?.getLanguageId() || '',
