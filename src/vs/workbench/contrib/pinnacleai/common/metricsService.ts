...
export class MetricsService implements IMetricsService {
	...
	constructor(
		...
		this.metricsService = ProxyChannel.toService<IMetricsService>(mainProcessService.getChannel('pinnacleai-channel-metrics'));
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'pinnacleaiDebugInfo',
			f1: true,
			title: localize2('pinnacleaiMetricsDebug', 'PinnacleAI: Log Debug Info'),
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		notifService.info(`PinnacleAI Debug info:\n${JSON.stringify(debugProperties, null, 2)}`)
	}
}) 