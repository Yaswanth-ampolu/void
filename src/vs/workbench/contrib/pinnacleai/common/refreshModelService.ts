...
import { RefreshableProviderName, refreshableProviderNames, SettingsOfProvider } from './pinnacleaiSettingsTypes.js';
...
const refreshBasedOn: { [k in RefreshableProviderName]: (keyof SettingsOfProvider[k])[] } = {
	...
	openAICompatible: ['_didFillInProviderSettings', 'endpoint'],
	liteLLM: ['_didFillInProviderSettings', 'endpoint'],
}
...
export class RefreshModelService extends Disposable implements IRefreshModelService {
	...
	constructor(
		@IPinnacleSettingsService private readonly pinnacleSettingsService: IPinnacleSettingsService,
		...
		const initializeAutoPollingAndOnChange = () => {
			...
			if (!pinnacleSettingsService.state.globalSettings.autoRefreshModels) return

			for (const providerName of refreshableProviderNames) {

				// const { '_didFillInProviderSettings': enabled } = this.pinnacleSettingsService.state.settingsOfProvider[providerName]
				this.startRefreshingModels(providerName, autoOptions)

				// every time providerName.enabled changes, refresh models too, like a useEffect
				let relevantVals = () => refreshBasedOn[providerName].map(settingName => pinnacleSettingsService.state.settingsOfProvider[providerName][settingName])
				let prevVals = relevantVals() // each iteration of a for loop has its own context and vars, so this is ok
				disposables.add(
					pinnacleSettingsService.onDidChangeState(() => { // we might want to debounce this
		...
		pinnacleSettingsService.waitForInitState.then(() => {
			initializeAutoPollingAndOnChange()
			this._register(
				pinnacleSettingsService.onDidChangeState((type) => { if (typeof type === 'object' && type[1] === 'autoRefreshModels') initializeAutoPollingAndOnChange() })
			)
		})
	...
	state: RefreshModelStateOfProvider = {
		...
		liteLLM: { state: 'init', timeoutId: null },
	}
	...
	startRefreshingModels: IRefreshModelService['startRefreshingModels'] = (providerName, options) => {
		...
		const autoPoll = () => {
			if (this.pinnacleSettingsService.state.globalSettings.autoRefreshModels) {
				// resume auto-polling
		...
		listFn({
			...
			onSuccess: ({ models }) => {
				...
				this.pinnacleSettingsService.setAutodetectedModels(
					providerName,
					models.map(model => {
						...
						else if (providerName === 'liteLLM') return (model as OpenaiCompatibleModelResponse).id;
						else throw new Error(`refreshMode fn: unknown provider ${providerName}`);
					}),
					{ enableProviderOnSuccess: options.enableProviderOnSuccess, hideRefresh: options.doNotFire }
				)

				if (options.enableProviderOnSuccess) this.pinnacleSettingsService.setSettingOfProvider(providerName, '_didFillInProviderSettings', true)
