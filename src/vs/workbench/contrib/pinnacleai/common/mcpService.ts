...
class MCPService extends Disposable implements IMCPService {
	...
	private async _initialize() {
		try {
			await this.pinnacleaiSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.pinnacleaiSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.pinnacleaiSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.pinnacleaiSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	// toggle MCP server and update isOn in pinnacleai settings
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		this._setMCPServerState(serverName, { status: 'loading', tools: [] })

		await this.pinnacleaiSettingsService.setMCPServerState(serverName, { isOn });
